...
//		System.out.println(String.format("add result: %s %s %s %.2f %.2f", re.playerWhite, re.playerBlack, re.pdnResult, probW, probB));
		if (re.getPdnResult().equals("1-0")) {
		...
		} else if (re.getPdnResult().equals("0-1")) {
	...
	protected int getInitialScore() {
		return CheckersPlugin.getInstance().getConfig().getInt("ladder.initial_position", INITIAL_POS);
	}
